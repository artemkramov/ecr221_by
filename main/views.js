...
var Modal = Backbone.View.extend({
    ...
    el:'#modalDialog',
    cancel: function() { events.trigger('clickDlg','hide'); },
    click: function(ev) {events.trigger('clickDlg',$(ev.target).data('ev')); },
    waitClick: function(buttons) {
        if (buttons) this.setButtons(buttons);
        var ret = new $.Deferred();
        this.listenTo(events,'clickDlg',function(btn){
            if ((btn=='cancel')||(btn=='hide')) { ret.reject(btn);
            } else { ret.resolve(btn);
            }
...
var LanguageSelector  = Backbone.View.extend({
    ...
    render: function() {
        _.each(schema.langs, function(value) {this.$el.append($("<option>").attr("value",value).text(value));},this);
...
var SummaryTable = Backbone.View.extend({
    ...
    render: function() {
        ...
        try { this.$el.html(this.template(this.model.toJSON()));
        } catch(e) {}
    ...
    logout:function(e){
        ...
        $.ajax('/cgi/state',{username:'logout'}).always(function(){$.get('/cgi/state');});
...
var MainCell = Backbone.View.extend({
    ...
    render: function() {
        ...
        if (addV) { this.$el.append(addV.render().$el);
        }
        return this;
    }
});
...
var NetworkView = AddView.extend({
    render: function() { this.$el.html(this.template({models:this.model.models})); return this; },
...
var LeftColumn = Backbone.View.extend({
    ...
    render: function() {
        ...
        if (this.addView) { this.$el.children().last().append(this.addView.render().$el);
        }
        return this;
    }
});
...
var FormDisplay = Backbone.View.extend({
    ...
    fill: function(isAll, def) {
        ...
        } else { this.setFormData(this.data.changed,def);
        }
    },
    ...
    formInObj: function (all) {
        ...
        $.each(f.elements,function(idx,el){
            ...
            switch(el.type) {
                ...
                case "checkbox":
                    ...
                    if (name in obj) { if (el.checked) obj[name] |= 1<<el.value;
                    } else {           obj[name]= el.checked?(1<<el.value):0;
                    }
        ...
        $.each(rem,function(idx,r){ if (r) delete obj[idx];
        });
        return obj;
    },
    clearFormData: function() {
        this.$(':input').each(function(idx,el){
            ...
            switch (el.prop('type')) {
                case "password": case "email": case "text": case "url": case "number": case "range":case "date":
                el.val('');
    ...
    setFormData: function(obj,def) {
        for (var name in obj) {
            ...
            switch (el.prop('type')) {
                ...
                case "select-one":
                    el.val(val);
                    if (def) { $.each(el[0].options, function(idx,o) { o.defaultSelected = o.index == el[0].selectedIndex;
                    });}
                    break;
                case "time": {
    ...
    render: function() {
        ...
        if (!this.data) {
            ...
            $.when(schema.tableFetch(name)).done( function() {
                ...
                if ($this.tbl) {
                    if ($this.tbl.length==0) { $this.tbl.newRow();
                    }
                ...
                $this.data.on('change',function(){$this.fill(false,false);},$this);
            });
        }
        return this;
    },
    onSync: function() { this.fill(false,true); },
    change: function(pos) {
        if (!this.tbl) return;
            ...
            switch(ev) {
                case 'refresh':
                    if (this.data) {
                        var $this=this;
                        events.trigger("buttonBlock:"+this.model.id,"refresh",true);
                        this.data.fetch().always(function(){events.trigger("buttonBlock:"+$this.model.id,"refresh",false);});
                    }
                    break;
                case 'last':this.change(this.tbl.length-1); break;
                case 'first':this.change(0); break;
                case 'next':this.change(this.row+1); break;
                case 'prev':this.change(this.row-1); break;
            }
        }
    }
});
...
var TableDisplay = Backgrid.Grid.extend({
    initialize: function(args) {
        ...
        this.$el.on('keydown',function(ev){
            if (ev.keyCode==46) {        $this.event('del');
            } else if (ev.keyCode==45) { $this.event('ins');
            }
        });
        this.listenTo(this.collection,"backgrid:selected",this.btnDel);
        this.listenTo(this.collection,"change",this.syncSave);
    },
    syncSave: function() {this.collection.syncSave();},
    btnDel: function() {
        ...
        if (!_.isUndefined(ev)) {
            switch(ev) {
                case 'refresh':{
                    var $this=this;
                    events.trigger("buttonBlock:"+this.model.id,"refresh",true);
                    this.collection.fetch().always(function(){events.trigger("buttonBlock:"+$this.model.id,"refresh",false);});
                } break;
            }
        }
    }
...
var PLUTableDisplay = TableDisplay.extend({
    ...
    event: function(ev) {
        if (!_.isUndefined(ev)) {
            var $this=this;
            switch(ev) {
                case 'del': this.collection.deleteRows(this.getSelectedModels()); break;
                case 'ins': this.collection.newRow(); break;
                case 'del-all': {
                    var $this=this;
                    var opt = {data:[]}
                ...
                default: TableDisplay.prototype.event.call(this,ev);
            }
        }
    },
    remove: function() {
        ...
        if (!_.isUndefined(ev)) {
            ...
            switch(ev) {
                ...
                case 'ins':
                    ...
                    this.change(0);
                    break;
                default: FormDisplay.prototype.event.call(this,ev);
            }
        }
    },
    remove: function() {
...
var LogoView = Backbone.View.extend({
    ...
    save: function(e) {
        ...
        $.ajax({
            ...
            if (resp.err) { $this.error('error',resp.err);
            }
    ...
    initialize: function(){
        ...
        this.rendered = false;
        this.loaded = false;
        if (this.img.src.endsWith('/cgi/logo.bmp')) { this.imageInit();
...
var TableContainer = Backbone.View.extend({
    ...
    errAlert: function (src,msg,field) {
        if (field) { msg = field+': '+msg; }
        if (src.collection) { msg = src.id+': '+msg; }
        this.insertAlert(msg);
    },
    click: function(ev) { if (this.content) this.content.event($(ev.currentTarget).data('ev')); },
    ...
    toggleData: function() {
        ...
        if ($('.navbar',this.$el).siblings().length) {
            ...
            var $this=this;
            $.when(schema.tableFetch(this.model.get('id'))).done( function() { $this.$el.append($this.content.render().$el)});
        }
    }
});
...
var Log = Backbone.View.extend({
    ...
    working: function() {return insertPoint;},
    /*events:{
...
var InterfacesTable = Backbone.View.extend({
    ...
    render: function() { this.$el.html(this.template({models:networkCell})); return this; }
});
