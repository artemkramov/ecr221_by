...
var Schema = Backbone.Collection.extend({
    ...
    CSVTable: function(name, inf, fname) {
        ...
        $.when(t).fail(function(xhr,status,thrown){
            ...
            var schCols = _.chain(sch.get('elems')).map(function(col){return col.name;}).union([key]).value();
            ...
            if (unkCol.length) { // some columns invalid
                ...
                if (tbl instanceof Backbone.Collection) { // Collection tbl
                    ...
                    if (err) { ret.reject(ok ? name : false);
                    } else { ret.resolve(name);
                    }
                    //tbl.syncSave();
                } else { // Form tbl
                    if (idIdx>=0) { // no id column in import file
    ...
    tableCSV: function(name) {
        if (name in this.cache) {
            ...
            var tbl = this.cache[name];
            var sch = this.get(name);
            var ro = _.map(_.filter(sch.get('elems'),function(c){return ('editable' in c) && !c.editable;}),function(e){return e.name});
            if (tbl instanceof Backbone.Collection) {
                if (tbl.length==0) return "";
                ro = _.without(ro,sch.get('key')||'id');
                ret+=_.keys(tbl.at(0).omit(ro)).join('\t')+'\r\n';
                tbl.each(function(v){ret+= _.values(v.omit(ro)).join('\t')+'\r\n';});
            } else {
    ...
    tableGroup: function(prefix) {
        return this.filter(function(el) { return el.get('prefix')==prefix; });
    },
    ...
    parseIn: function(type,val) {
        switch(type.type) {
            ...
            case "percent":
            case "qty":
            case "number": if (_.isString(val)) {
                return (type.step && (type.step<1))?parseFloat(val):parseInt(val);
            } break;
        }
        return val;
    },
    fixIn: function(tbl,obj) { this.fix(tbl,obj,tbl.parseCol,this.parseIn); },
    fix:function(tbl,obj,col,parse) {
        if (col) _.each(_.intersection(_.keys(obj),col),function(e){ obj[e] = parse(this.typeCol(tbl,e),obj[e]);},this);
    },
    ...
    fixOut: function(tbl,obj) { this.fix(tbl,obj,tbl.syncCol,this.parseOut); },
    typeCol: function(tbl,field) {
        return _.find(tbl.get('elems'),function(el){ return el.name==field; });
    },
    ...
    parseError: function(err,callback) {
        var parseErrorInner = function(e,cb) {
            ...
            if (!_.has(e,'e')) return;
            if (_.has(e,'p')) { msg = schema.error(e.e, e.p);
            } else { msg = schema.error(e.e);
            }
            cb(msg, e.f);
        };
        if (_.isArray(err)) { _.each(err, _.partial(parseErrorInner,_,callback));
        } else { parseErrorInner(err,callback);
        }
    ...
    str:function(id) { return (this.tr && this.tr.str && this.tr.str[id]) || id;
    },
    fixupTable:function(tbl) {
        ...
        _.each(tbl.get('elems'),function(el){
            ...
            switch(el.type) { // table type conversion
                case 'number': el.cell='integer'; return;
                case 'text':   el.cell='string'; return;
                case 'date':   el.cell='date'; return;
                case 'url':    el.cell='uri'; return;
                case 'select-one':
                case 'radio':
                    el.cell = Backgrid.SelectCell.extend({
                        optionValues: _.map(extractLabels(el.labels),function(v,k){return [v,k];}),
                        formatter: RadioFormatter
                    });
                    return;
                case 'checkbox':
                    el.cell = Backgrid.SelectCell.extend({
                        optionValues: _.map(extractLabels(el.labels),function(v,k){return [v,Math.pow(2,k)];}),
                        multiple:true,
...
_.extend(CheckFormatter.prototype, {
    toRaw: function (d, model) {
        return _.isArray(d) ? _.reduce(d,function(memo,num){ return memo+Number(num); },0): (_.isNull(d)?0:d);
    }
});
