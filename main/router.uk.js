...
var AppRouter = Backbone.Router.extend({
    ...
    repScr: function() { this.view = new ReportPage(); },
    logoScr: function() { this.view = new LogoView(); },
    tableScr: function() { this.view = new GroupTable({group:'cfg'});},
    networkTab:0,
    networkScr: function(page) {
        switch(page) {
            case "state": this.networkTab=0; break;
            case "settings": this.networkTab=1;break;
        }
    ...
    modemScr: function(page) {
        switch(page) {
            case "state": this.modemTab=0; break;
            case "settings": this.modemTab=1;break;
            case "docs": this.modemTab=2;break;
        }
        this.view = new PagesScreen({no:this.modemTab,models:modemPages});
    },
    fisc: function(page) {
        switch(page) {
            case "fisc": this.fiscTab=0; break;
            case "time": this.fiscTab=1;break;
            case "reset": this.fiscTab=2;break;
        }
...
var appStart = function(){
    ...
    $.get('/cgi/mdm_info').done(function(){gprsExists=true;}).always(function(){
        networkCell.refresh().always(function(){qryDone.resolve();});
    });
    schema.load(function() {schemaLoaded.resolve();});
    $.when(qryDone,schemaLoaded).always(function(){
        if (schema.get('PLU')) {
            mainScreenCells.unshift(new MainCell({model:new Backbone.Model(
                {lnk:'#plu',img:'plu',name:'PLU'
                    //addView: new PLUView()
                })}));
        }
        if (schema.get('Logo')) {
            mainScreenCells.push(new MainCell({model:new Backbone.Model(
        ...
        if (gprsExists) { networkViews.push(new GPRSState());
        }
        networkPages = [
